...
exports.minidump = exports.taboowordcheck = exports.mergeddataload = exports.ghostdataload = exports.rivaldataload = exports.musicdataload = exports.playerdataload = exports.playerdatasave = exports.playerdatanew = void 0;
...
function loadSongCache() {
    ...
    for (const sw of [...world_1.SONGS_WORLD, ...world_1.SONGS_OVERRIDE_WORLD]) {
        ...
        if (!existing) {
            ...
            _songCache.set(sw.mcode, Object.assign(Object.assign({}, existing), { diffLv: sw.diffLv }));
        }
    }
            ...
            try {
                ...
                while ((m = musicRegex.exec(xml)) !== null) {
                    ...
                    _songCache.set(mcode, {
                        title: title || (existing === null || existing === void 0 ? void 0 : existing.title) || `ID ${mcode}`,
                        artist: artist || (existing === null || existing === void 0 ? void 0 : existing.artist) || '',
                        basename: basename || (existing === null || existing === void 0 ? void 0 : existing.basename) || '',
                        diffLv: useXmlDiffLv ? xmlDiffLv : ((existing === null || existing === void 0 ? void 0 : existing.diffLv) || []),
                    });
                }
            }
            catch (e) {
...
function loadCoreDbForDDR() {
    ...
    try {
        ...
        for (const dbPath of candidates) {
            ...
            if (isSqlite) {
                try {
                    ...
                    const rows = db.prepare("SELECT data, __refid FROM documents WHERE __s IN ('card','user_account') OR _collection IN ('card','user_account')").all();
                    for (const row of rows) {
                        try {
                            const doc = JSON.parse(row.data);
                            if (doc.__s === 'card' && row.__refid && doc.cid)
                                cards.push({ cid: doc.cid, __refid: row.__refid });
                            else if (doc.__s === 'user_account' && doc.cardNumber && doc.countryCode)
                                userCountry.set(doc.cardNumber, doc.countryCode);
                        }
                        catch (_a) { }
                    }
                    db.close();
                    for (const c of cards) {
                        ...
                        if (cc)
                            countryByRefid.set(c.__refid, cc);
                    }
                }
                catch (_b) { }
            }
                ...
                for (const line of lines) {
                    ...
                    try {
                        ...
                        if (doc.__s === 'card' && doc.__refid && doc.cid)
                            cards.push(doc);
                        else if (doc.__s === 'user_account' && doc.cardNumber && doc.countryCode)
                            userCountry.set(doc.cardNumber, doc.countryCode);
                    }
        ...
        console.warn('[DDR Discord] Could not load core.db for flags:', e.message);
    }
}
...
function withFlag(name, refid) {
    const cc = countryByRefid.get(refid);
    const flag = cc ? getFlagEmoji(cc) : '';
    return flag ? `${flag} ${name}` : name;
}
async function discordAutoExport(playerName, refid, songId, style, difficulty, clearKind, score, exScore, oldScore, previousDancerName, previousRefid) {
    ...
    let jacketBuffer = null;
    const jacketDir = U.GetConfig("ddr_jacket_dir");
...
function getLastGhostId(ghost) {
    let ghostFiltered = ghost.filter(a => (a.ghostId !== undefined));
    if (ghostFiltered.length > 0)
        return ghostFiltered.sort((a, b) => b.ghostId - a.ghostId)[0].ghostId;
    else
        return 0;
}
async function saveScores(refid, hiScoreInfo, songId, style, difficulty, rank, clearKind, score, exScore, maxCombo, flareForce, ghostSize, ghost) {
    let stepScore = await DB.FindOne(refid, { collection: "score3", songId: songId, style: style, difficulty: difficulty });
    let ghostData = await DB.Find(null, { collection: "ghost3" });
    ...
    if (lastGhostId === 0) {
        ...
        ghostId = lastGhostId += 1;
    // Save score + ghost data
    await DB.Insert(refid, {
        collection: "scorelog",
        songId,
        style,
        difficulty,
    ...
    // Hiscore cleanup - save new PB ghostId instead of overwriting existing ghostData 
    let hiScoreWorld1 = await DB.FindOne(null, { collection: 'hiscore3', slot: 1, songId: songId, style: style, difficulty: difficulty });
    if (!hiScoreWorld1 || hiScoreCheck >= hiScoreWorld1.score) {
        const prevTop = hiScoreWorld1;
        await DB.Upsert(refid, { collection: "hiscore3", slot: 1, songId, style, difficulty }, { $set: { country, region, customerCode, companyCode, locationId, pcbid, dancerName, score: hiScoreCheck, rank, clearKind, ghostId } });
        // Notify Discord
        discordAutoExport(dancerName, refid, songId, style, difficulty, clearKind, hiScoreCheck, exScore, prevTop ? prevTop.score : -1, prevTop ? (prevTop.dancerName || '') : '', prevTop ? prevTop.__refid || '' : '').catch(e => console.error('[DDR Discord]', e));
    }
    let hiScoreArea1 = await DB.FindOne(null, { collection: 'hiscore3', slot: 2, locationId: locationId, songId: songId, style: style, difficulty: difficulty });
    if (!hiScoreArea1 || hiScoreCheck >= hiScoreArea1.score) {
        await DB.Upsert(refid, { collection: "hiscore3", slot: 2, locationId, songId, style, difficulty }, { $set: { country, region, customerCode, companyCode, pcbid, dancerName, score: hiScoreCheck, rank, clearKind, ghostId } });
    }
    let hiScoreMachine1 = await DB.FindOne(null, { collection: 'hiscore3', slot: 3, locationId: locationId, pcbid: pcbid, songId: songId, style: style, difficulty: difficulty });
    if (!hiScoreMachine1 || hiScoreCheck >= hiScoreMachine1.score) {
        await DB.Upsert(refid, { collection: "hiscore3", slot: 3, locationId, pcbid, songId, style, difficulty }, { $set: { country, region, customerCode, companyCode, dancerName, score: hiScoreCheck, rank, clearKind, ghostId } });
    }
}
const playerdatanew = async (info, data, send) => {
    const refid = $(data).str("data.refid");
    let ddrCode = _.random(1, 99999999);
    await DB.Upsert(refid, { collection: "profile3" }, {
        collection: "profile3",
        ddrCode: ddrCode
    });
    return send.object({
...
exports.playerdatanew = playerdatanew;
const playerdatasave = async (info, data, send) => {
    const refid = $(data).str("data.refid");
    if (!refid.startsWith("X000")) {
        if ($(data).number("data.savekind") === 1) {
            ...
            let profile = await DB.FindOne(refid, { collection: "profile3" });
            await DB.Upsert(refid, { collection: "profile3" }, {
                collection: "profile3",
                ddrCode: $(data).number('data.common.ddrcode'),
                dancerName: $(data).str('data.common.dancername'),
            ...
            if (eventData) {
                for (const e of eventData) {
                    ...
                    await DB.Upsert(refid, { collection: "event3", eventId: eid, eventNo: eno, eventType: etype }, {
                        collection: "event3",
                        eventId: eid,
                        eventNo: eno,
                        eventType: etype,
            ...
            if (leagueData) {
                ...
                let leagueExist = await DB.FindOne(refid, { collection: 'league3', id: leagueId });
                await DB.Upsert(refid, { collection: "league3" }, {
                    $set: {
                        id: leagueId,
            ...
            if (braveData) {
                await DB.Upsert(refid, { collection: "profile3" }, { $set: {
                ...
                for (const det of braveData.elements('detail')) {
                    // unset last play
                    await DB.Update(refid, { collection: "galaxybrave", braveId: det.number('braveid'), lastPlay: true }, { $set: { lastPlay: false } });
                    await DB.Upsert(refid, { collection: "galaxybrave", braveId: det.number('braveid'), pointer: det.number('pointer') }, { $set: {
...
exports.playerdatasave = playerdatasave;
const playerdataload = async (info, data, send) => {
    const refid = $(data).str("data.refid");
    let profile = await DB.FindOne(refid, { collection: "profile3" });
    if (!profile || !profile.dancerName || refid.startsWith("X000")) {
        return send.object({
            ...
            event: world_1.EVENTS_GUEST_WORLD.map(e => ({
        ...
        const scores = await DB.Find(refid, { collection: "score3" });
        let scoreFin = [];
        if (scores) {
            for (const scoreData of scores) {
                let mcodeIndex = scoreFin.findIndex(x => $(x).number('mcode') === scoreData.songId);
                const playCount = await DB.Count(refid, { collection: "scorelog", songId: scoreData.songId, style: scoreData.style, difficulty: scoreData.difficulty });
                if (mcodeIndex < 0) {
                    let scr = {};
                    ...
                    scr['score_double'] = [];
                    /*
                      difficulty,playcount,grade,clearkind,score,ghostid,flaredisp,flarepoints,idk
                      needs more work, 9 vals
                    */
                    scr[(scoreData.style === 0) ? 'score_single' : 'score_double'] = [
                        {
                            score_str: K.ITEM('str', `${scoreData.difficulty},${playCount + 1},${scoreData.rank},${scoreData.clearKind},${scoreData.score},${scoreData.ghostId},${scoreData.flareForce},${scoreData.flareForce}`)
                        }
                    ];
                    scoreFin.push(scr);
                }
        ...
        let eventFin = [];
        let eventData = await DB.Find(refid, { collection: "event3" });
        ...
        let curLeague = world_1.LEAGUE_WORLD.find(lg => lg.id === world_1.LEAGUE_SEASON);
        let prevLeague;
        let leagueData = {};
        let leagueInfo;
        let leagueResult;
        ...
        let lResult = [];
        let playerLeague = await DB.Find(refid, { collection: 'league3' });
        if (playerLeague.length > 0) {
            ...
            if (playerLeague.length > 1)
                await DB.Remove(refid, { collection: "league3", $not: { id: leagueInfo.id } });
            ...
            if (leagueInfo.id !== curLeague.id) {
                prevLeague = world_1.LEAGUE_WORLD.find(lg => lg.id === leagueInfo.id);
                leagueResultPrev = await DB.FindOne({ collection: 'leagueresult3', id: leagueInfo.id, class: leagueClass > 0 ? leagueClass : 1 });
                if (leagueClass === 1 && leagueInfo.score >= leagueResultPrev.promoteScore)
                    resultClass += 1;
                else if (leagueClass === 2) {
                    if (leagueInfo.score >= leagueResultPrev.promoteScore)
                        resultClass += 1;
                    else if (leagueInfo.score < leagueResultPrev.demoteScore)
                        resultClass -= 1;
                }
                else if (leagueClass === 3 && leagueInfo.score < leagueResultPrev.demoteScore)
                    resultClass -= 1;
                if (!leagueInfo.ended) {
                    lResult.push({
        ...
        leagueResult = await DB.FindOne({ collection: 'leagueresult3', id: curLeague.id, class: leagueClass > 0 ? leagueClass : 1 });
        if (leagueInfo) {
            if (leagueClass === 1 && leagueInfo.score >= leagueResult.promoteScore)
                resultClass += 1;
            else if (leagueClass === 2) {
                if (leagueInfo.score >= leagueResult.promoteScore)
                    resultClass += 1;
                else if (leagueInfo.score < leagueResult.demoteScore)
                    resultClass -= 1;
            }
            else if (leagueClass === 3 && leagueInfo.score < leagueResult.demoteScore)
                resultClass -= 1;
            ended = (leagueInfo.ended !== undefined) ? leagueInfo.ended : false;
        }
        if (BigInt(Date.now()) >= curLeague.start)
            leagueStatus = 1;
        if (BigInt(Date.now()) >= curLeague.end)
            leagueStatus = 2;
        if (BigInt(Date.now()) >= curLeague.summary) {
            ...
            if (leagueInfo && !ended) {
                ...
                await DB.Upsert(refid, { collection: 'league3' }, { $set: { ended: true } });
            }
        }
        leagueData = {
        ...
        let userCustomize = [];
        let customize = await DB.Find(refid, { collection: 'customize3' });
        const defCustIds = [
        ...
        customize.forEach(cus => {
            var _a;
            const randomCust = ((_a = cus.random) === null || _a === void 0 ? void 0 : _a.length) === 0 ? defCustIds[cus.category - 1] : cus.random;
            userCustomize.push({
        ...
        let playerBraveData = await DB.Find(refid, { collection: 'galaxybrave' });
        for (const brave of world_1.GALAXY_BRAVE) {
        ...
        // add new properties
        if (profile.fsQuickmode === undefined)
            profile.fsQuickmode = false;
        if (profile.subscribed === undefined)
            profile.subscribed = false;
        if (profile.subscribePopupEnable === undefined)
            profile.subscribePopupEnable = false;
        if (profile.subscribePopupDisable === undefined)
            profile.subscribePopupDisable = false;
        if (profile.opSpeedType === undefined)
            profile.opSpeedType = 0;
        if (profile.opRealSpeed === undefined)
            profile.opRealSpeed = 0;
        if (profile.opLanePreview === undefined)
            profile.opLanePreview = 0;
        if (profile.opComboPriority === undefined)
            profile.opComboPriority = 0;
        if (profile.opJudgePriority === undefined)
            profile.opJudgePriority = 0;
        if (profile.opJudgePosition === undefined)
            profile.opJudgePosition = 0;
        if (profile.opTimingMusic === undefined)
            profile.opTimingMusic = 0;
        if (profile.lpTabMainGraphType === undefined)
            profile.lpTabMainGraphType = 0;
        if (profile.lpTabMainGraphDisp === undefined)
            profile.lpTabMainGraphDisp = 0;
        if (profile.lpTabSubGraphType === undefined)
            profile.lpTabSubGraphType = 0;
        if (profile.lpTabSubGraphDisp === undefined)
            profile.lpTabSubGraphDisp = 0;
        if (profile.fsClearType === undefined)
            profile.fsClearType = 0;
        if (profile.fsDifficulty === undefined)
            profile.fsDifficulty = 0;
        const rivals = await DB.Find(refid, { collection: 'rival3', slot: { $gt: 0 } });
        return send.object(Object.assign({ result: K.ITEM("s32", 0), refid: K.ITEM("str", refid), gamesession: K.ITEM('s64', BigInt(1)), servertime: K.ITEM("u64", BigInt(getDate())), is_locked: K.ITEM('bool', false), common: {
                ...
                slot: K.ITEM("s32", r.slot),
                rivalcode: K.ITEM("s32", r.rivalCode)
            })), score: scoreFin, event: eventFin, customize: userCustomize, brave: galaxyBrave }, (U.GetConfig("world_league") && {
            league: leagueData
        })));
    }
};
exports.playerdataload = playerdataload;
const musicdataload = async (info, data, send) => {
    ...
    if (IO.Exists('webui/uploads/mdb_limited.xml')) {
        ...
        for (const music of mdb['mdb']['music']) {
            ...
            let overrideIndex = world_1.SONGS_OVERRIDE_WORLD.findIndex(s => s.mcode === mcode);
            if (overrideIndex > -1) {
                limitedAry = (world_1.SONGS_OVERRIDE_WORLD[overrideIndex]['limited_ary'] && world_1.SONGS_OVERRIDE_WORLD[overrideIndex]['limited_ary'].length > 0 ? world_1.SONGS_OVERRIDE_WORLD[overrideIndex]['limited_ary'] : limitedAry);
                difficultyArr = world_1.SONGS_OVERRIDE_WORLD[overrideIndex]['diffLv'];
            }
            let inLim = limited;
    ...
    for (const music of world_1.SONGS_WORLD) {
        if (blacklist.includes(music.mcode))
            continue;
        let limArr = music.limited_ary;
        if (music.mcode === world_1.LEAGUE_GOLD_BORDER_MCODE && BigInt(Date.now()) >= world_1.LEAGUE_WORLD.find(lg => lg.id === world_1.LEAGUE_SEASON)['summary']) {
            for (let i in limArr)
                if (limArr[i] === 14)
                    limArr[i] = limArr[i] - 1;
        }
...
exports.musicdataload = musicdataload;
const rivaldataload = async (info, data, send) => {
    var _a, _b;
    ...
    const profiles = await DB.Find(null, { collection: 'profile3' });
    // song id, style, difficulty, idk, dancername, idk, idk, scoredisp, score, ghost
    let record = [];
    let hiscore;
    if (ddrCode === 0) {
        if (loadKind === 1) {
            hiscore = await DB.Find(null, { collection: 'hiscore3', slot: loadKind });
        }
        else if (loadKind === 2) {
            hiscore = await DB.Find(null, { collection: 'hiscore3', slot: loadKind, locationId: locationId });
        }
        else if (loadKind === 3) {
            hiscore = await DB.Find(null, { collection: 'hiscore3', slot: loadKind, locationId: locationId, pcbid: pcbid });
        }
        for (const hsi of hiscore) {
            const prf = profiles.find(p => p['__refid'] === hsi['__refid']);
            record.push({
                record_str: K.ITEM('str', `${hsi.songId},${hsi.style},${hsi.difficulty},${prf.ddrCode},${prf.dancerName},${prf.area},${(_a = hsi === null || hsi === void 0 ? void 0 : hsi.rank) !== null && _a !== void 0 ? _a : 0},${(_b = hsi === null || hsi === void 0 ? void 0 : hsi.clearKind) !== null && _b !== void 0 ? _b : 0},${hsi.score},${hsi.ghostId}`)
            });
        }
    }
    else {
        const rival = await DB.FindOne(null, { collection: 'profile3', ddrCode });
        if (rival) {
            hiscore = await DB.Find(rival['__refid'], { collection: 'score3' });
        }
        for (const hsi of hiscore) {
            record.push({
...
exports.rivaldataload = rivaldataload;
const ghostdataload = async (info, data, send) => {
    const refid = $(data).str("data.refid");
    const ghostId = $(data).number("data.ghostid");
    let ghostData = await DB.FindOne(null, { collection: 'ghost3', ghostId: ghostId });
    if (ghostData) {
        return send.object({
            result: K.ITEM("s32", 0),
...
exports.ghostdataload = ghostdataload;
const mergeddataload = async (info, data, send) => {
    let mergedData = {
        leagueClass: 0,
        advanceBorder: false,
        subscribed: false,
    ...
    for (const player of players) {
        let curLeague = world_1.LEAGUE_WORLD.find(lg => lg.id === world_1.LEAGUE_SEASON);
        let playerProf = await DB.FindOne(null, { collection: 'profile3', ddrCode: player.number('ddrcode') });
        let playerLeague = await DB.FindOne(playerProf['__refid'], { collection: 'league3' });
        let tempEvent = await DB.Find(playerProf['__refid'], { collection: "event3" });
        ...
        if (playerLeague) {
            ...
            if (playerLeague.class > mergedData.leagueClass)
                mergedData.leagueClass = playerLeague.class;
            if (playerLeague.score >= border)
                mergedData.advanceBorder = true;
        }
        if (playerProf.subscribed)
            mergedData.subscribed = true;
...
exports.mergeddataload = mergeddataload;
const taboowordcheck = async (info, data, send) => {
...
exports.taboowordcheck = taboowordcheck;
const minidump = async (info, data, send) => {
    return send.object({ result: K.ITEM('s32', 0) });
};
exports.minidump = minidump;
function getDate() {
    let time = new Date();
    let tempDate = time.getDate();
    const currentTime = parseInt((time.getTime() / 100000)) * 100;
    return currentTime;
}
const loadEventData = async (eventData, refid) => {
    let result = [];
    for (const event of world_1.EVENTS_WORLD) {
        ...
        if (event.dep) {
            event.dep.forEach(dep => {
                if (eventData.find(e => e.eventId === dep) === undefined)
                    condmet = false;
            });
        }
        if (event.type === 201) {
        ...
        else if ([70, 71, 72, 73, 74, 81, 82, 83, 84].includes(event.type))
            compTime = (eData && eData.compTime !== 0) ? eData.compTime : 0;
        ...
        if (condmet) {
            if (event.type === 9999 && event.id === 101 && !U.GetConfig('world_league'))
                continue;
            result.push({
                id: event.id,
                type: event.type,
