...
// Real song indices extracted from the game's music_list.xml (576 songs, max index 632)
// Loaded lazily on first request to avoid blocking module init
let _op3MusicIndices: number[] | null = null;
async function getOp3MusicIndices(): Promise<number[]> {
  if (!_op3MusicIndices) {
    const json = await IO.ReadFile('data/op3_music_indices.json', 'utf-8');
    _op3MusicIndices = JSON.parse(json);
  }
  return _op3MusicIndices!;
}
...
export const get_music_info: EPR = async (info, data, send) => {
  ...
  // Use real song indices from music_list.xml (576 songs, ~83KB in KBin = within 100KB buffer)
  const musicIndices = await getOp3MusicIndices();
  for (const i of musicIndices) {
    music_spec.push(K.ATTR({ index: `${i}` }, {
      ...
      unlock_type: K.ITEM('s8', 1),
      real_unlock_type: K.ITEM('s8', 1),
    }));
  }
